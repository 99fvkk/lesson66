...
def delete_book(conn, title, author):
    ...
    if not id_1:
        print('Нельзя удалить книгу, ее нет в биьлиотееке')
        return False
    id_1=id_1[0]
...
def add_reader(conn, full_name, phone, age):
    ...
    cursor.execute('''INSERT INTO readers (pr, full_name, phone, age) VALUES (?, ?, ?, ?)''',
...
def add_book(conn, title, author, genre, n):
    ...
    if not max_id:
        max_id=0
    cursor.execute('''INSERT INTO books (id, title, author, genre, total, free) 
                           VALUES (?, ?, ?, ?, ?, ?)''',
                   (max_id+1, title, author, genre, n, n))
    conn.commit()
    return True